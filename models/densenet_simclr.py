...
class DenseNetSimCLR(nn.Module):
    def __init__(self, depth, num_classes, growth_rate=12, out_dim=128,
        ...
        self.relu = nn.ReLU(inplace=True)
        # classifier
        self.classify_head = nn.Linear(in_planes, num_classes)
        self.in_planes = in_planes
        # contrastive learning head
        self.contrastive_head = nn.Sequential(
            nn.Linear(in_planes, in_planes),  # projector
            nn.ReLU(),
            nn.Linear(in_planes, out_dim)
        )
    ...
    def forward(self, x):
        ...
        return self.classify_head(out), self.contrastive_head(out)
